...
L1 = ['заявката', 'заявката', 'заявката', 'заявката',
      'заявката']  # 'заявката', ne me kefqt
L2 = ['заявката', 'заявьата', 'завякатва', 'вя', 'язвката']  # 'заявкатаа',
C = [0, 1, 2, 7, 3]  # , 1
O = [[('з', 'з'),  ('а', 'а'),  ('я', 'я'),  ('в', 'в'),  ('к', 'к'),  ('а', 'а'),  ('т', 'т'),  ('а', 'а')], [('з', 'з'),  ('а', 'а'),  ('я', 'я'),  ('в', 'в'),  ('к', 'ь'),  ('а', 'а'),  ('т', 'т'),  ('а', 'а')], [
    ('з', 'з'),  ('а', 'а'),  ('яв', 'вя'),  ('к', 'к'),  ('а', 'а'),  ('т', 'т'),  ('', 'в'),  ('а', 'а')]]  # [('з', 'з'),  ('а', 'а'),  ('я', 'я'),  ('в', 'в'),  ('к', 'к'),  ('а', 'а'),  ('т', 'т'),  ('', 'а'),  ('а', 'а')]
D = [22.75, 32.06, 35.93, 62.03, 43.71]  # 32.02, kopie ot vtori arg 32.06

# Тест на editDistance
for s1, s2, d in zip(L1, L2, C):
    assert a1.editDistance(
        s1, s2) == d, "Разстоянието между '{}' и '{}' следва да е '{}'".format(s1, s2, d)
print("Функцията editDistance премина теста.")

# Тест на editOperations
for s1, s2, o in zip(L1, L2, O):
    assert a1.editOperations(
        s1, s2) == o, "Операциите редактиращи '{}' до '{}' следва да са '{}'".format(s1, s2, o)
...
fullSentCorpusOriginal = [[w.lower() for w in sent]
...
operationProbs = a1.computeOperationProbs(
    fullSentCorpusOriginal, fullSentCorpusTypos)
print('Готово.')
ps = [operationProbs[k] for k in operationProbs.keys()]

assert max(
    ps) < 0.2, "Не би следвало  да има елементарна операция с толкова голяма вероятност."
...
for s1, s2, d in zip(L1, L2, D):
    assert abs(a1.editWeight(s1, s2, operationProbs) -
               d) < 1, "Теглото между '{}' и '{}' следва да е приблизително '{}'".format(s1, s2, d)
print("Функцията editWeight премина теста.")

# Тест на generate_edits
assert len(set(a1.generateEdits("тест"))
           ) == 269, "Броят на елементарните редакции \"тест\"  следва да е 269"
...
assert len(set(a1.generateCandidates("такяива", dictionary, operationProbs))
           ) == 4, "Броят на генерираните кандидати следва да е 4"
print("Функцията generateCandidates премина теста.")

# Тест на correct_spelling
a1.correctSpelling(fullSentCorpusTypos[3668:3669], dictionary, operationProbs)
corr = a1.correctSpelling(
    fullSentCorpusTypos[3668:3669], dictionary, operationProbs)
...
corpus_corrected = a1.correctSpelling(
    fullSentCorpusCorrect, dictionary, operationProbs)
