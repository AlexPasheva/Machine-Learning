...
def computeOperationProbs(corrected_corpus, uncorrected_corpus, smoothing=0.2):
    ...
    for index in range(len(uncorrected_corpus_words)):
        result = editOperations(
            uncorrected_corpus_words[index-1], corrected_corpus_words[index-1])
        for op in result:
            if op in operations:
                operations[op] += 1
    ...
    for i in range(1, rows):
        for j in range(1, cols):
            d[i][j] = min(d[i-1][j-1] + operationWeight(s2[i-1], s1[j-1], operationProbs),
                          d[i-1][j] +
                          operationWeight(s2[i - 1], '', operationProbs),
                          d[i][j-1] +
                          operationWeight('', s1[j - 1], operationProbs)
                          )

            if i > 1 and j > 1 and s1[j - 2] == s2[i - 1] and s1[j - 1] == s2[i - 2] and s1[j-2:j] != s2[i-2:i]:
                swap_op = (
                    d[i - 2][j - 2] + (operationWeight(s1[j-2:j], s2[i-2:i], operationProbs)))
...
def generateEdits(q):
    ...
    replaces = [L + c + R[1:]
                for L, R in splits if R for c in alphabet if c != R[0]]
...
def correctSpelling(r, dictionary, operationProbs):
    ...
    for sentence in r:
        ...
        for word in sentence:
            cyrillic = [x for x in word if x in alphabet]
            if len(cyrillic) == len(word) and word not in dictionary:
                candidates = dict(generateCandidates(
                    word, dictionary, operationProbs))
                if candidates:
                    candidate = min(candidates, key=candidates.get)
                    correctedSentence.append(candidate)
            else:
                correctedSentence.append(word)

        correctedQuery.append(correctedSentence)
